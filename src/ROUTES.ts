...
const ROUTES = {
    ...
    POLICY_ACCOUNTING_CERTINIA_PREREQUISITES: {
        ...
        // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
        getRoute: (policyID: string, isSandbox?: boolean, backTo?: string) =>
            getUrlWithBackToParam(`workspaces/${policyID}/accounting/certinia/prerequisites/${isSandbox?.toString() ?? ''}` as const, backTo),
    },
